...
export default function BottomPlayer() {
  ...
  return (
    ...
    >
      ...
      {/* Left: Song Info */}
      <div className="w-full md:w-1/3 md:flex-1 min-w-0 flex items-center justify-center md:justify-start gap-3 md:pr-2">
        {/* Chevron Up hint */}
        ...
        <div className="relative w-14 h-14 rounded-md overflow-hidden shadow-lg border border-white/10">
        ...
        <div className="block ml-2 w-full overflow-hidden text-left">
            <h4 className="text-white font-medium truncate max-w-full md:max-w-[150px]">{currentSong.title}</h4>
      ...
      <div className="w-full md:w-1/3 flex flex-col items-center gap-1 md:gap-2 max-w-[500px]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full flex items-center gap-2 text-xs text-white/50 font-mono">
            <span className="hidden sm:block">{formatTime(currentTime)}</span>
             <div 
            ...
            <span className="hidden sm:block">{currentSong ? formatTime(currentSong.duration) : "0:00"}</span>
        </div>
      </div>
